...
describe('end-to-end', function() {
  ...
  describe('with npm', function() {
    ...
    before(function() {
      ...
      return run('npm', ['pack'], { stdio: 'pipe' }).then(({ stdout: filename }) => {
        return rename(filename, path.join(tmpDir, 'ember-electron.tgz'));
      }).then(() => {
        process.chdir(tmpDir);
        ...
        return ember('install', `ember-electron@file:${tmpDir}/ember-electron.tgz`);
      });
    });
